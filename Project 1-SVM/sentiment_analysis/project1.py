...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    for i in range(len(labels)):
        hinge_loss_full = hinge_loss_full + (hinge_loss_single(feature_matrix[i], labels[i], theta, theta_0))
    hinge_loss_full = hinge_loss_full / len(labels)
...
def pegasos_single_step_update(
    ...
    if label * (np.dot(current_theta, feature_vector) + current_theta_0) <= 1:
        update_theta = (mult_eta * current_theta) + (eta * label * feature_vector)
        update_theta_0 = current_theta_0 + (eta * label)
    else:
        update_theta = mult_eta * current_theta
...
def classify(feature_matrix, theta, theta_0):
    ...
    #(nsamples, nfeatures) = feature_matrix.shape
    predicted = np.empty(feature_matrix.shape[0])
    for i in range(feature_matrix.shape[0]):
        feature_vector = feature_matrix[i]
        predict = np.dot(theta, feature_vector) + theta_0
        if (predict > 0):
            predicted[i] = 1
        else:
            predicted[i] = -1
...
def classifier_accuracy(
    ...
    training_predictions = classify(train_feature_matrix, theta, theta_0)
    validation_predictions = classify(val_feature_matrix, theta, theta_0)
    training_accuracy = accuracy(training_predictions, train_labels)
...
def extract_bow_feature_vectors_count(reviews, dictionary):
    """
    Inputs a list of string reviews
    Inputs the dictionary of words as given by bag_of_words
    Returns the bag-of-words feature matrix representation of the data.
